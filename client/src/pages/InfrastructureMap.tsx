...
import { parseKmlFile, type KmlPreviewItem } from "@/lib/map/kml";
import React, { useState, useRef, useCallback, useEffect, useMemo } from "react";
import { Sheet, SheetContent, SheetHeader, SheetTitle } from "@/components/ui/sheet";

// ─── Motor do mapa, extraído deste arquivo ──────────────────────────────────
// Estes módulos saíram daqui sem alteração de comportamento. O objetivo é
// permitir que outras telas de mapa reaproveitem a mesma lógica em vez de
// copiá-la — foi a duplicação que manteve o mobile com zoom divergente do
// desktop por meses.
import { safeLeafletRemove } from "@/lib/map/leaflet-utils";
import { haversineDistance, formatDistance } from "@/lib/map/geo";
import { FIBER_VIA_COLORS, STATUS_COLOR, createLeafletIcon } from "@/lib/map/icons";
import type { MapRoute, MapPoi, SidePanelContent } from "@/lib/map/types";
import ResizableDetailPanel from "@/components/map/ResizableDetailPanel";
import TubeSelectors from "@/components/map/TubeSelectors";
import MapContextMenu, { type MapContextMenuTarget } from "@/components/map/MapContextMenu";
...
export default function InfrastructureMap() {
  ...
  const parseKmlToPreview = useCallback(async (file: File): Promise<void> => {
    ...
    try {
      const items = await parseKmlFile(file);
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-border bg-card/50 flex-wrap">
        ...
        {isAdmin && (
          <>
            ...
            <div className="w-px h-4 bg-border mx-1" />
            {/* ── Menu Adicionar Elemento (sub-componente para evitar erro React #185) ── */}
            <AddElementDropdown
              addingMode={addingMode}
              addingRouteMode={addingRouteMode}
              addingOltMode={addingOltMode}
              addingDgoMode={addingDgoMode}
              addingPoleMode={addingPoleMode}
              addingReserveMode={addingReserveMode}
              addingPoiMode={addingPoiMode}
              setAddingMode={setAddingMode}
              setAddingRouteMode={setAddingRouteMode}
              setAddingOltMode={setAddingOltMode}
              setAddingDgoMode={setAddingDgoMode}
              setAddingPoleMode={setAddingPoleMode}
              setAddingReserveMode={setAddingReserveMode}
              setAddingPoiMode={setAddingPoiMode}
