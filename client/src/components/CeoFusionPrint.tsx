 ...
 * Renderiza o mapa de fusões em layout espelhado (TUBO A ↔ TUBO B).
 *
 * Lógica de pareamento:
 *  - Para cada via do TUBO A que está fusionada com o TUBO B,
 *    exibimos a linha: VIA_A | label_A | ↔ | label_B | VIA_B
 *  - Vias sem fusão aparecem com "—" no lado oposto.
 *  - Pares de tubos são determinados pelo grafo de fusões:
 *    dois tubos formam um par se há ao menos uma fusão entre eles.
 *  - Tubos sem nenhuma fusão aparecem em bloco simples no final.
 */
export function CeoFusionPrint({ ceo, tubes, allVias, roomName }: CeoFusionPrintProps) {
  ...
  // Cada par é { tubeA, tubeB } onde tubeA < tubeB (por índice no array).
  // Tubos sem parceiro ficam em `soloTubes`.
  const pairedSet = new Set<number>(); // ids de tubos já emparelhados
  ...
  for (let i = 0; i < tubes.length; i++) {
    ...
    // Encontrar o tubo com mais fusões com tubeA
    ...
    // Escolher o parceiro com mais fusões que ainda não foi emparelhado
  ...
  // ── Índice de tubos (para o cabeçalho) ────────────────────────────────────
  ...
  // ── Renderização de um par espelhado ──────────────────────────────────────
  function renderPair(tubeA: Tube, tubeB: Tube) {
    ...
    // Construir mapa de fusões A→B e B→A
    // Para cada linha i (1..maxVias), tentamos encontrar a via A e a via B
    const rows: {
      viaA: Via | null;
      viaB: Via | null;
      isFused: boolean;
    ...
    // Adicionar vias B que não foram emparelhadas com nenhuma via A
    for (const vB of viasB) {
      if (!usedBIds.has(vB.id)) {
        rows.push({ viaA: null, viaB: vB, isFused: false });
      }
    }
    ...
    return (
      <div key={`${tubeA.id}-${tubeB.id}`} className="tube-section">
        ...
        <div style={{
          ...
          {/* Tubo A */}
          <div style={{
            background: tubeA.type === "splitter" ? "#fef3c7" : "#e0f2fe",
            border: `1px solid ${tubeA.type === "splitter" ? "#f59e0b" : "#0891b2"}`,
            borderRadius: "3px",
            padding: "2mm 3mm",
          }}>
            ...
            <div style={{ fontSize: "7.5pt", color: "#6b7280", marginTop: "0.5mm" }}>
              {tubeA.totalVias} vias · {fusedA} fusionada{fusedA !== 1 ? "s" : ""}
              {tubeA.color ? ` · ${tubeA.color}` : ""}
            </div>
          </div>
          ...
          {/* Tubo B */}
          <div style={{
            background: tubeB.type === "splitter" ? "#fef3c7" : "#e0f2fe",
            border: `1px solid ${tubeB.type === "splitter" ? "#f59e0b" : "#0891b2"}`,
            borderRadius: "3px",
            padding: "2mm 3mm",
          }}>
            ...
            <div style={{ fontSize: "7.5pt", color: "#6b7280", marginTop: "0.5mm" }}>
              {tubeB.totalVias} vias · {fusedB} fusionada{fusedB !== 1 ? "s" : ""}
              {tubeB.color ? ` · ${tubeB.color}` : ""}
            </div>
          </div>
        </div>
            ...
            {rows.map((row, idx) => {
              ...
              return (
                <tr key={idx} style={{ background: rowBg }}>
                  ...
                  <td>
                    {viaA?.label
                      ? <span style={{ fontWeight: 500 }}>{viaA.label}</span>
                  ...
                  <td>
                    {viaB?.label
                      ? <span style={{ fontWeight: 500 }}>{viaB.label}</span>
                      : <span style={emptyStyle}>—</span>}
                  </td>
                  {/* VIA B */}
  ...
  // ── Renderização de tubo solo (sem par) ───────────────────────────────────
            ...
            {vias.map(via => {
              ...
              return (
                <tr key={via.id}>
                  ...
                  <td style={{ textAlign: "center" }}>
                    {isFused ? (
                      <span style={{ background: "#cffafe", color: "#0891b2", padding: "1px 5px", borderRadius: "3px", fontSize: "7pt", fontWeight: 700 }}>
                        FUSIONADA
                      </span>
                    ) : (
                      <span style={{ background: "#f3f4f6", color: "#9ca3af", padding: "1px 5px", borderRadius: "3px", fontSize: "7pt" }}>
                        LIVRE
                      </span>
                    )}
                  </td>
  ...
  // ── Dividir conteúdo em páginas (2 pares ou 3 solos por página) ────────────
  type PageItem =
    | { kind: "pair"; tubeA: Tube; tubeB: Tube }
    | { kind: "solo"; tube: Tube };
            ...
            <>
              <div className="print-header">
                <div>
                  ...
                  {roomName && (
                    <div style={{ fontSize: "9pt", color: "#6b7280" }}>🏢 {roomName}</div>
                  )}
                  {ceo.notes && (
              ...
              <div style={{
                ...
                <div style={{ fontWeight: 700, fontSize: "8pt", color: "#374151", marginBottom: "1.5mm" }}>
                  ÍNDICE DE TUBOS / SPLITTERS
                </div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "2mm" }}>
                  {indexEntries.map(({ tube, fused, total }) => (
                    <div key={tube.id} style={{
                      ...
                      gap: "1.5mm",
                      padding: "1mm 2.5mm",
                      background: tube.type === "splitter" ? "#fef3c7" : "#e0f2fe",
                      border: `1px solid ${tube.type === "splitter" ? "#f59e0b" : "#7dd3fc"}`,
                      borderRadius: "3px",
                      fontSize: "7.5pt",
                    }}>
                      <span style={{ fontWeight: 700, color: tube.type === "splitter" ? "#92400e" : "#0c4a6e" }}>
                        {tube.identifier}
                      </span>
                      ...
                      <span style={{
                        background: fused === total ? "#d1fae5" : fused > 0 ? "#fef3c7" : "#f3f4f6",
                        color: fused === total ? "#065f46" : fused > 0 ? "#92400e" : "#6b7280",
                        padding: "0 3px",
                        borderRadius: "2px",
                        fontSize: "6.5pt",
                        fontWeight: 700,
