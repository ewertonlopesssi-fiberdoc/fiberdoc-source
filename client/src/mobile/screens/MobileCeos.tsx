            ...
            <div className="flex items-center gap-2">
              ...
              {isOnline() && (
                <>
                  <button
                    onClick={async () => {
                      // Recarregar dados estruturais
                      await Promise.all([
                        loadTubes(selected.id),
                        loadBandejas(selected.id),
                        loadAllVias(selected.id),
                        loadSplitters(selected.id),
                      ]);
                      // Recarregar vias de todos os tubos em paralelo para atualizar barras de ocupação
                      const freshTubes = await client.ceoTubes.byCeo.query({ ceoId: selected.id }).catch(() => tubes);
                      const newCache = new Map<number, Via[]>();
                      await Promise.all(
                        (freshTubes as unknown as typeof tubes).map(async (tube) => {
                          try {
                            const vias = await client.ceoVias.byTube.query({ tubeId: tube.id });
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    const freshVias = await client.ceoVias.byTube.query({ tubeId: selectedTube.id });
                    ...
                    if (selectedVia.fusedToTubeId && selectedVia.fusedToTubeId !== selectedTube.id) {
                      try {
                        const destVias = await client.ceoVias.byTube.query({ tubeId: selectedVia.fusedToTubeId });
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const freshSrc = await client.ceoVias.byTube.query({ tubeId: selectedTube.id });
                        ...
                        if (parseInt(fusionTubeId) !== selectedTube.id) {
                          try {
                            const freshDest = await client.ceoVias.byTube.query({ tubeId: parseInt(fusionTubeId) });
